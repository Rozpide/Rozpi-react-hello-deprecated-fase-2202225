...
const getState = ({ getStore, getActions, setStore }) => {
    return {
        ...
        actions: {
            ...
            getFavPriceData: (id) => {
                fetch(`https://api.coingecko.com/api/v3/coins/${id}/market_chart?vs_currency=${getStore().currency}&days=${getStore().timeFrame}`)
                    .then((res) => res.json())
                    .then((response) => {
                        setStore({
                            favoritePriceData:  
            ...
            addToFavs: (coin) => {
                fetch(`https://psychic-potato-7vvw4xvvrw7934xw-3001.app.github.dev/favorites/${coin.id}`, {
                    ...
                    .then(response => getActions().getFavoriteIds(getStore().userID))
				...
				fetch(`https://psychic-potato-7vvw4xvvrw7934xw-3001.app.github.dev/users/${id}/favorites`)
					...
					.then((response) => {
                        ...
                        response.map((favorite) =>
                            {getActions().getFavoriteData(favorite.coin_id)}
                        )
                    })
					.catch((err) => console.log(err))
			},
            getFavoriteData: (coin_id) => {
                fetch(`https://api.coingecko.com/api/v3/coins/${coin_id}?localization=false&tickers=false&market_data=false&community_data=false&developer_data=false&sparkline=false`)
                    .then((res) => res.json())
